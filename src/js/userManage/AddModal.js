...
class AddModal{
	...
	initMD(type,method="create"){
		...
		const tit =method ==="create" ? type === "view" ?  "创建视图" : "创建目录" :"复制视图";
	...
	handle(){
	  ...
	  this.$addMView.on("click",function() {
		...
		this.$addMBtn.click(function(){
			...
			const cur_id =  $("#section").attr("curid");
...
class IssueMView{
  ...
  orgBoxInit(){
	...
	handle(){
      ...
      const $issueMView = _self.$issueMView ;
			const id= +$("#section").attr("curid");
		...
		//开关
		this.$issueMView.on("click",".s-switch",function(){
