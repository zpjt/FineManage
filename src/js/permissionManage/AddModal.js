...
class AddModal{
	...
	GroupKpiByDim(kpi){
		...
		api.GroupKpiByDim({kpi}).then(res=>{
			if(res){
				...
				const {pub_ids,data:kpiData} = res ;
				this.dimN = pub_ids.join(","); // 指标的维度值类型
			
				
				const dimDataArr = [];
				const dimStr = pub_ids.map(dimData=>{

						const  {dim_name,dim_value,dim_id,sub}  = this.dimData.find(val=>val.dim_id == dimData);
						dimDataArr.push(sub);
						return ` <div >
												<p style="color:#2B95BA">${dim_name}</b></p>
												<div class="s-comboBox" style="margin:8px 0"> </div>
										</div>
										`;

				});
				const zbStr = this.dimN && `<p class="s-title"><b class="inp-tip">*</b>主题维度</p><div>${dimStr.join("")}</div>` ||"";
				$dimBox.html(zbStr);

				$.map($dimBox.find(".s-comboBox"),(dimcombox,index)=>{

						this.dimCombo[index] = new SCombobox($(dimcombox),{
						"data":dimDataArr[index],
						"textField":"dim_name",
						"idField":"dim_value",
						 "width":320,
				...
				// 回填维度
			 	const dimX = this.dimX ;  //指标的维度值,用于回填
				if(this.optId && dimX &&  dimX !== "null"){
					  const dimXArr= dimX.split(",");
						this.dimCombo.forEach((val,index)=>{
							val.setValue(dimXArr[index]);
							val.box.find(".no-fill").removeClass("no-fill");
						}); 
				} 
	...
	getAllParams(){
		  ...
		  const dim2 = this.orgTreeCombo.box.find(".combo-value").val(),
			      dimN = this.dimN,
		        dimX = this.dimN && $.map(this.$dimBox.find(".combo-value"),val=>val.value).join(",");
